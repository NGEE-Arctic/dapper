...
def e5lh_to_elm(csv_directory, write_directory, df_loc, remove_leap=True, id_col=None):
    """
    Batched version; handles multiple sites.
    """
    ...
    for i, f in enumerate(files):
        ...
        for gid, this_df in ppdfg:
            parquet_path = temp_path / f"{gid}.parquet"
...
# def export_for_elm_gridded(df, lon, lat, elm_write_dir, dformat='BYPASS', compress=True, compress_level=4):
#     """
#     Export in ELM-ready foramts.
#     df has all the data. Sorted by date already.
#     df_loc has a list of points (pids) and their locations (lat, lon).
